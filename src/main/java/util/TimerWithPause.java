...
public abstract class TimerWithPause {
	 ...
	 * @param duration
	 *            The period in millis for which the timer should run. Set it to
	 *            {@code Timer#DURATION_INFINITY} if the timer has to run
	 *            indefinitely.
	 */
	public TimerWithPause(long interval, long duration) {
	...
	public void start() {
		...
		future = execService.scheduleWithFixedDelay(new Runnable() {
			...
			public void run() {
				onTick();
				elapsedTime += TimerWithPause.this.interval;
				if (duration > 0) {

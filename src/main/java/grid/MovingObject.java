...
public abstract class MovingObject extends GridObject {
	...
	/**
	 * Test for GridObjects in the given square that prevent this MovingObject from moving into it.
	 * @param square Square to inspect for obstructions.
	 * @return Null if a move to the given square is possible, or the obstruction that prevents it.
	 */
	 ...
	 * Only one moving object can be in a given square at a time.
	 * @param grid A grid to contain this object.
	 */
	public MovingObject(Grid grid, GridPoint location, Direction direction) {
		super(grid, location);
	...
	/**
	 * Wrapper for move(int, Direction) to allow moves relative to the current direction.
	 ...
	 */
	public void move(int steps, RelativeDirection relativeDirection) throws InterruptedException,
			ImpossibleActionException, UnavailableActionException {
		this.move(steps, relativeDirection.toAbsolute(direction));
	}
	 ...
	 * @throws ImpossibleActionException 
	 */
	public void move(int steps, Direction moveDirection) throws InterruptedException, 
			ImpossibleActionException, UnavailableActionException {
		if (steps < 0) {
			throw new IllegalArgumentException("Cannot move a negative number of steps.");
		}
		if (!direction.equalsDirection(moveDirection)) {
			new Turn(this, moveDirection).execute();
		}
		Move forwardsMove = new Move(this);
	...
	public GridPoint getNearestPoint(RelativeDirection relativeDirection) {
		if (relativeDirection.equals(RelativeDirection.here)) {
			return new GridPoint((int)Math.round(x), (int)Math.round(y));
		} else {
			return new GridPoint(
					(int)Math.round(x + relativeDirection.toAbsolute(direction).getXComponent()),
					(int)Math.round(y + relativeDirection.toAbsolute(direction).getYComponent()));
		}
	}
}
...
public class Direction {
	...
	/**
	 * Construct a mutable Direction with the given initial angle, measured
	 * clockwise from north
	 *
	 * @param radians angle in radians
	 */
	public Direction(final double radians) {
		this.mutable = true;
		this.radians = normaliseAngle(radians);
	}

	private Direction(final double radians, final boolean mutable) {
	...
	public void setValue(final double value) {
		if (!this.mutable) {
			throw new IllegalArgumentException("Cannot set the value of a constant direction!");
		}
	...
	public boolean equalsDirection(final Direction other) {
		final double difference = Math.abs(other.getRadians() - this.radians);
	...
	/**
	 * Get the y component of this direction. Note that we use 'computer' (as
	 * opposed to mathematical) y coordinates; y increases south.
	 *
	 * @return a double representing the y component of this direction, from -1 to
	 *         1.
	 */
	...
	public String toString() {
		...
		if (equalsDirection(north)) {
			return "North";
		} else if (equalsDirection(east)) {
			return "East";
		} else if (equalsDirection(south)) {
			return "South";
		} else if (equalsDirection(west)) {
			return "West";
		} else {
			return "Unknown";
		}
	...
	public Direction round() {
		final int n = (int) (Math.round(this.radians / (Math.PI * 0.5)) % 4);
		switch (n) {
		case 0:
			return north;
		case 1:
		...
		case 3:
			return west;
		default:
			throw new RuntimeException("Failed to round " + this.radians + " rad to NSEW!");
		}
	}
	 ...
	 *
	 * @return a static final direction representing one of the four points of the
	 *         compass.
	 */

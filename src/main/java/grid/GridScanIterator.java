 ...
 * etc. Returns only the squares that are not empty.
 */
public class GridScanIterator extends GridIterator implements Iterator<Square> {
	public GridScanIterator(final Square[][] array) {
		super(array);
		reset();
	}
	...
	// Advance the pointer 0 or more steps, until a non-null, non-empty square is
	// found
	private void walk() {
		while (!this.done && (this.array[this.x][this.y] == null || this.array[this.x][this.y].getCount() == 0)) {
			step();
		}
	}
}

...
public abstract class Action {
	...
	public void execute() throws InterruptedException, ImpossibleActionException, UnavailableActionException {
		...
		try {
			// Initialise can throw Impossible or Unavailable exceptions but if so, always cleans up after itself
			initialise();
	...
	private void waitOneStep() throws InterruptedException {
		long nextStepTime =	startedAt + (currentStep * duration() / numberOfSteps());
	...
	/**
	 * Work to be performed immediately when the action is executed, before the start event is fired.
	 * Can be empty if no special setup is needed.
	 * If the action is impossible or unavailable, implementations MUST clean up before throwing an exception.
	...
	/**
	 * Work to be performed at the end of each step of the action, such as updating the model.
	 * For all action steps except the last, a step event is fired immediately after this method returns.
	 */
	protected abstract void executeOneStep();
	
	/**
	 * Work to be performed after the last action step has completed, before the end event is fired.
	 * Can be empty if no special cleanup is needed.
	 */

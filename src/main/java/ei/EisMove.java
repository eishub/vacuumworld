...
public class EisMove extends AbstractEISEntityAction {
	...
	public Percept act(String entity, Action action) throws ActException {
		...
		if (action.getParameters().size() == 1) {
			return move(vacBotEntity.bot, action.getParameters().get(0).toProlog(), 1);
		} else if (action.getParameters().size() >= 2) {
			return move(vacBotEntity.bot, action.getParameters().get(0).toProlog(), 
	...
	private Percept move(VacBot vacBot, String directionName, int steps) throws ActException {
		...
		if (directionName.equals(Direction.NORTH)) {
			...
			throw new ActException(ActException.FAILURE,
					"Invalid direction \"" + directionName + "\".");
		}
		
		try {
			vacBot.move(steps, directionToMove);
		} catch (InterruptedException e) {
			throw new ActException(ActException.FAILURE, "Move was interrupted!");
		} catch (ImpossibleActionException e) {
			throw new ActException(ActException.FAILURE, "Move permanently impossible in this environment.");
		} catch (UnavailableActionException e) {
			throw new ActException(ActException.FAILURE, "Move temporarily unavailable - is another VacBot in the way?");
		}
